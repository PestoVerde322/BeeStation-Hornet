...
/obj/item/noogie/attack(mob/living/carbon/target, mob/living/carbon/human/user)
	if(!istype(target))
		to_chat(user, "<span class='warning'>You can't give this a noogie!</span>")
		return
	...
	if(!IS_ORGANIC_LIMB(target.get_bodypart(BODY_ZONE_HEAD)))
...
/obj/item/noogie/proc/noogie_loop(mob/living/carbon/human/user, mob/living/carbon/target, iteration)
	...
	user.adjustStaminaLoss(iteration + 10)
