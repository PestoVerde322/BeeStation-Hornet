...
/obj/machinery/station_map/proc/setup_holomap()
	...
	floor_markings = image('icons/holomap/stationmap.dmi', "decal_station_map")
...
/obj/machinery/station_map/proc/open_map(var/mob/user)
	...
	playsound(src, 'sound/holomap/holomap_open.ogg', 125)
...
/obj/machinery/station_map/proc/close_map()
	...
	playsound(src, 'sound/holomap/holomap_close.ogg', 125)
...
/obj/machinery/station_map/engineering/handle_overlays()
	...
	for(var/obj/machinery/firealarm/alarm as anything in GLOB.station_fire_alarms["[current_z_level]"])
		if(alarm?.z == current_z_level && alarm?.myarea?.fire)
			var/image/alarm_icon = image('icons/holomap/8x8.dmi', "fire_marker")
	...
	if(length(fire_alarms))
		extra_overlays["Fire Alarms"] = list("icon" = image('icons/holomap/8x8.dmi', "fire_marker"), "markers" = fire_alarms)
	...
	for(var/obj/machinery/airalarm/air_alarm in GLOB.machines)
		...
		if(air_alarm?.z == current_z_level && alarms?.atmosalm) //Altered it to fire_alam since we don't have an area variable on air_alarms
			var/image/alarm_icon = image('icons/holomap/8x8.dmi', "atmos_marker")
	...
	if(length(air_alarms))
		extra_overlays["Air Alarms"] = list("icon" = image('icons/holomap/8x8.dmi', "atmos_marker"), "markers" = air_alarms)
