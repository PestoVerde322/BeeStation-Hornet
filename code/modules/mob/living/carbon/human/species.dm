...
/datum/species/proc/handle_hair(mob/living/carbon/human/human_host, forced_colour)
	...
	if(human_host.head)// Wig stuff
		if(istype(human_host.head, /obj/item/clothing/head/wig))
			worn_wig = human_host.head
		if(istype(human_host.head, /obj/item/clothing/head))
			var/obj/item/clothing/head/hat = human_host.head
			if(hat.attached_wig)
	...
	if(human_host.wear_mask)
		var/obj/item/I = human_host.wear_mask
		if(isclothing(I))
			var/obj/item/clothing/C = I
	...
	if(!hair_hidden || dynamic_hair_suffix || worn_wig)
		...
		else if((human_host.hair_style && (HAIR in species_traits)) || worn_wig)
			var/current_hair_style = human_host.hair_style
			var/current_hair_color = human_host.hair_color
			var/current_gradient_style = human_host.gradient_style
			var/current_gradient_color = human_host.gradient_color
			if(worn_wig)
...
/datum/species/proc/handle_body(mob/living/carbon/human/human_host)
	...
	if(HAS_MARKINGS in species_traits)
		var/obj/item/bodypart/chest/chest = human_host.get_bodypart(BODY_ZONE_CHEST)
		var/obj/item/bodypart/r_arm/right_arm = human_host.get_bodypart(BODY_ZONE_R_ARM)
		var/obj/item/bodypart/l_arm/left_arm = human_host.get_bodypart(BODY_ZONE_L_ARM)
		var/obj/item/bodypart/r_leg/right_leg = human_host.get_bodypart(BODY_ZONE_R_LEG)
		var/obj/item/bodypart/l_leg/left_leg = human_host.get_bodypart(BODY_ZONE_L_LEG)
		var/datum/sprite_accessory/markings = GLOB.moth_markings_list[human_host.dna.features["moth_markings"]]
		var/markings_icon_state = markings.icon_state
		if(ismoth(human_host) && HAS_TRAIT(human_host, TRAIT_MOTH_BURNT))
...
/datum/species/proc/handle_mutant_bodyparts(mob/living/carbon/human/human_host, forced_colour)
	...
	var/g = (human_host.dna.features["body_model"] == FEMALE) ? "f" : "m"
...
//what's the purpose of this if then it's undefined with nothing? here it used to be the whole movement delay, where did it go?
...
/datum/species/proc/spec_attacked_by(obj/item/I, mob/living/user, obj/item/bodypart/affecting, intent, mob/living/carbon/human/human_host)
	...
	if((I.damtype == BRUTE) && (I.force >= max(10, armor_block) || I.is_sharp()))
		...
		if(Iforce > 10 || Iforce >= 5 && prob(33))
			human_host.force_say(user)	//forcesay checks stat already.
	return TRUE
...
/datum/species/proc/on_hit(obj/projectile/P, mob/living/carbon/human/human_host)
...
/datum/species/proc/bullet_act(obj/projectile/P, mob/living/carbon/human/human_host)
...
/datum/species/proc/stop_wagging_tail(mob/living/carbon/human/human_host)
	var/obj/item/organ/tail/tail = human_host?.getorganslot(ORGAN_SLOT_TAIL)
	tail?.set_wagging(human_host, FALSE)
...
/datum/species/proc/z_impact_damage(mob/living/carbon/human/human_host, turf/T, levels)
	human_host.apply_general_zimpact_damage(T, levels)
	if(levels < 2)
		return
	// SPLAT!
	// 5: 50%, 4: 32%, 3: 18%
	...
	if(prob(min((levels - 1) * 15, 75)))
		if(levels >= 3 && prob(25))
			for(var/selected_part in list(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM, BODY_ZONE_L_LEG, BODY_ZONE_R_LEG))
				var/obj/item/bodypart/bp = human_host.get_bodypart(selected_part)
				if(bp)
					bp.dismember()
			return
